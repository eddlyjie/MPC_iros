...
class MPC_Solver:
    ...
    def __init__(self, T, N, vehicle_params, track_params, mpc_costs, state_bounds=None, control_bounds=None):
        ...
        Args:
            T (float): Sampling time for the MPC.
            N (int): The prediction horizon.
            vehicle_params (dict): A dictionary containing the vehicle's physical parameters.
            track_params (dict): A dictionary containing track information, like the .mat file.
            mpc_costs (dict): A dictionary of cost weights for the objective function.
        """
        self.T = T
        self.N = N
        ...
        # Create the solver instance and define variable bounds
    ...
    def _create_solver(self):
        ...
        This internal method contains the core optimization problem definition.
        """
        ...
        P = ca.SX.sym('P', self.n_states + 5 * maxNumBlocks + 10) # Parameters: x0, block_info, cost_poly
        
        obj = 0  # Objective function
        g = []   # Constraint vector

        # Unpack parameters and costs for easier access
        ...
        # Extract block and cost polynomial parameters from the parameter vector P
        block_params_flat = P[self.n_states : self.n_states + 5 * maxNumBlocks]
        cost_para = P[self.n_states + 5 * maxNumBlocks:]
        ...
        for k in range(self.N):
            ...
            # Add dynamics constraint using RK4 integration
        ...
        # Add terminal cost to incentivize reaching the goal
        ...
        # Set up the NLP problem and solver options
        ...
        opts = {
            'ipopt': {
                ...
                "warm_start_init_point": "yes", "print_level": 0, "sb" : "yes"
            },
            'print_time': 0
        }
    ...
    def _set_solver_bounds(self):
        ...
        # You should load these from your config file
        # STATE_BOUNDS = self.vehicle_params.get('STATE_BOUNDS', [(-ca.inf, ca.inf)] * self.n_states)
        # CONTROL_BOUNDS = self.vehicle_params.get('CONTROL_BOUNDS', [(-ca.inf, ca.inf)] * self.n_controls)
    ...
    def get_solution(self, x0, u0, X0_pred):
        """
        Solves the NMPC problem for a given initial state and initial guesses.
        
        Args:
            x0 (np.ndarray): The current state of the vehicle.
            u0 (casadi.DM): An initial guess for the control inputs over the horizon.
            X0_pred (casadi.DM): An initial guess for the state trajectory over the horizon.
            
        Returns:
            A dictionary from CasADi containing the solver's output, or None if it fails.
        """
        # Determine the local track section and progress polynomial
        cur_pose = x0[0:2].T
        ...
        # unwrap yaw angles for continuity
        # self.track_data['block_info'][indices, 2] = np.unwrap(self.track_data['block_info'][indices, 2], discont=np.pi)
        # print("indices for local blocks:", indices)
        # print("yaw of local blocks:", self.track_data['block_info'][indices, 2])
        # print("yaw of vehicle:", x0[4])
        ...
        # Pack the parameters and initial guess for the solver
        p = ca.vertcat(x0, ca.reshape(local_blocks, -1, 1), cost_poly)
        x_init = ca.vertcat(ca.reshape(X0_pred, -1, 1), ca.reshape(u0, -1, 1))
        
        # Call the solver and handle potential errors
        try:
    ...
    def solve_and_update(self, x0):
        """
        Solves the NMPC problem using the previous solution as a warm start.
        This method manages the initial guesses for states and controls internally.
        
        Args:
            x0 (np.ndarray or casadi.DM): The current state of the vehicle.
            
        Returns:
            A dictionary from CasADi containing the solver's output, or None if it fails.
        """
        # Initialize guesses if this is the first run
        if self.u0 is None or self.X0_pred is None:
            self.u0 = ca.DM.zeros((self.n_controls, self.N))
            # Ensure x0 is a CasADi DM for repmat
            current_x0_dm = ca.DM(x0) if isinstance(x0, np.ndarray) else x0
            self.X0_pred = ca.repmat(current_x0_dm, 1, self.N + 1)

        # Call the core solver function with the stored initial guesses
        solution = self.get_solution(x0, self.u0, self.X0_pred)
        
        # If successful, update the internal guesses for the next iteration (warm start)
        if solution is not None:
            # Extract the optimal control sequence and predicted state trajectory
            ...
            # Update for the next iteration by shifting the control sequence
            # self.u0 = ca.horzcat(u_optimal[:, 1:], ca.reshape(u_optimal[:, -1], -1, 1))
            self.u0 = u_optimal
            self.X0_pred = X_optimal # Use the new optimal trajectory as the next guess
